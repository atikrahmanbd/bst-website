...
const SoftwareDevelopmentHeader = () => (
  <div className="absolute inset-0 p-6 flex flex-col">
    ...
    <CardTitle className="text-xl font-bold text-neutral-800 dark:text-neutral-100">
...
const SEOMarketingHeader = () => (
  <div className="absolute inset-0 flex flex-col justify-between">
    ...
    <div className="p-6 h-40 relative z-10">
      ...
      <p className="mt-2 max-w-xs text-sm text-neutral-600 dark:text-neutral-300">
    ...
    <div className="relative h-full w-full overflow-hidden z-10">
      <div className="mt-2 ml-6 h-full w-full rounded-tl-lg border border-neutral-200 bg-neutral-100/50 dark:border-neutral-700 dark:bg-neutral-800/50 backdrop-blur-sm p-4">
        <Image
...
const ITSolutionsHeader = () => {
  ...
  return (
    <div className="absolute inset-0 flex flex-col p-6 dark:bg-black/50">
      <motion.div
        initial="initial"
        animate="animate"
        whileHover="hover"
        className="flex flex-1 w-full h-full min-h-[6rem] dark:bg-dot-white/[0.2] bg-dot-black/[0.2] flex-row space-x-2 mb-2"
      >
        <motion.div
          variants={first}
      ...
      <h3 className="text-xl font-bold text-neutral-800 dark:text-neutral-100 mt-4 mb-2">
        IT &amp; Custom Solutions
      </h3>
...
const WebDesignStep = ({ title }: { title: string }) => {
  return (
    <li className="flex gap-3 items-start">
      <CircleCheckBig className="h-5 w-5 text-blue-500 mt-0.5 shrink-0" />
      <p className="text-base text-neutral-700 dark:text-white">{title}</p>
    </li>
  );
};
...
const WebDesignHeader = () => (
  <div className="absolute inset-0">
    <CardSpotlight className="h-full w-full bg-white dark:bg-black/80 border-neutral-200 dark:border-neutral-800 p-10">
      <h3 className="text-2xl font-bold text-neutral-800 dark:text-neutral-100 mb-4 relative z-20">
        <GradientText>Web Design</GradientText>
      </h3>
      <p className="text-base text-neutral-600 dark:text-neutral-300 mb-4 relative z-20">
...
export function AdditionalServices() {
  ...
  return (
    <section className="relative pt-0 pb-20 px-4">
        ...
        <BentoGrid className="max-w-7xl mx-auto">
          {items.map((item, i) => (
            <div
              key={i}
              className={cn(
                "group/bento shadow-input row-span-1 rounded-xl border border-neutral-200 bg-white transition duration-200 hover:shadow-xl dark:border-white/[0.2] dark:bg-card dark:shadow-none relative overflow-hidden",
                item.className
              )}
            >
              {item.header}
