...
async function fetchSpotlightDomains(): Promise<
  { extension: string; price: string; tag?: string | null }[]
> {
  try {
    ...
    if (spotlightDomainsFromApi.length >= 6) {
      return spotlightDomainsFromApi;
    }
    ...
    return FALLBACK_TLDS.slice(0, 6).map((tld) => ({
      extension: tld,
...
export async function DomainHero() {
  const spotlightDomains = await fetchSpotlightDomains();

  return <DomainHeroClient spotlightDomains={spotlightDomains} />;
}
