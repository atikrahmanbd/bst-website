...
interface HomeServicesOfferedClientProps {
  totalDomains: number;
}

export function HomeServicesOfferedClient({
  totalDomains,
}: HomeServicesOfferedClientProps) {
  return (
              ...
              <div className="absolute top-0 left-0 right-0 py-4 px-2 sm:p-6 md:p-8 z-[30] text-center md:text-left">
                ...
                >
                  <span className="sm:inline">
                    Domains - {totalDomains}+ Extensions
                  </span>
                  <ArrowRight className="size-5 sm:size-6 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300" />
                </Link>
                <p className="text-muted-foreground font-normal mt-2 text-sm sm:text-base">
                  Choose From Over {totalDomains} Domain Extensions To Secure
                  Your Online Identity With Instant Registration and Activation
                </p>
              </div>
